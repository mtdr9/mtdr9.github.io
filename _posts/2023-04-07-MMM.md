...
As mentioned, a multi-armed bandit test would be the ideal way to find the optimal allocation. But given that it's a 3 variable problem, I'm going to take some shortcuts. First, we can use 2 clues to justify nearly removing newspaper from our budget: the random forest assigns nearly 0 feature importance to it, meaning it's not a significant driver of performance. This aligns with our correlation EDA.
...
![png](http://mattdorros.com/img/MMM/rf_importances.png)

Second, if we assign a few trial budgets to TV and radio, toggling newspaper has some effect on sales up to $75, although it's minimal.

```python
# test newspaper's impact given static TV and radio spend
np_trials = pd.DataFrame({'TV': [100]*len(list(range(0-ns, 301-ns, 5))),
             'radio': [50]*len(list(range(0-ns, 301-ns, 5))),
             'newspaper': list(range(0-ns, 301-ns, 5))})

plt.plot(list(range(0-ns, 301-ns, 5)),rf_model.predict(np_trials))
plt.xlabel('Newspaper spend')
plt.ylabel('Sales')
```

![png](http://mattdorros.com/img/MMM/np_spend.png)

So we'll pass values of newspaper up to $75, and TV and radio up to $400, such that the total is never greater than $400.
...
* Feature selection: while testing model error, or once we've selected a model, it's often helpful to prune features that aren't contributing to its predictive power. By looking at feature importance, we can see that newspaper is basically uncorrelated with sales; so it's a likely candidate to prune.
